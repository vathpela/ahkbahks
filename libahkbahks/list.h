...
static inline bool __list_add_valid(struct list_head *new UNUSED,
				    struct list_head *prev UNUSED,
				    struct list_head *next UNUSED)
{
	return true;
}
static inline bool __list_del_entry_valid(struct list_head *entry UNUSED)
{
	return true;
}
	  ...
	  void *state)
{
	...
	array = calloc(ALIGN(1, pgcnt), sizeof(void *));
	if (!array)
		return -1;

	list_for_each(pos, head) {
		struct list_head **new_array;
		size_t newmemb = ALIGN(nmemb+1, pgcnt);
